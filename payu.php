...
class plgVmPaymentPayu extends vmPSPlugin {
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		$post_variables = Array(
                    'descripcion'       => $method->payu_description,
                    'prueba'            => $method->debug,
                    'usuarioId'         => $method->payu_user_id,
                    'refVenta'          => $order['details']['BT']->order_number,
                    'valor'             => number_format(floatval($totalInPaymentCurrency), '2','.',''),
                    'iva'               => $iva,
                    'baseDevolucionIva' => $baseDevolucionIva,
                    'moneda'            => $method->payu_currency,
